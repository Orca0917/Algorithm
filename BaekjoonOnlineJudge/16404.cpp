...
int lazy_propagation(int node, int start, int end) {
...
int query_tree(int node, int start, int end, int index) {
...
int main() {
    ...
    for (int person = 1; person <= n; ++person) {
        int boss; cin >> boss;#include <bits/stdc++.h>
#define ll long long
#define pii pair<int, int>
#define MAX 100001
using namespace std;
...
int main() {
    ...
    while (m--) {
        ...
        if (option == 1) {
            int person, value; cin >> person >> value;
            update_tree(1, 1, n, range[person].first, range[person].second, value);
        }
        else {
            int person; cin >> person;
            cout << query_tree(1, 1, n, range[person].first) << "\n";
        }
    }
    return 0;
}